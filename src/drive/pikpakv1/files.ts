...
import {CONFIG_INFO, SAVING_INFO, PikPakFile} from "./metas";
...
export class HostDriver extends BasicDriver {
    ...
    private async getFiles(parentId: string): Promise<PikPakFile[]> {
        ...
        let pageToken = "";
        do {
            const params = new URLSearchParams({parent_id: parentId, thumbnail_size: "SIZE_MEDIUM", with_audit: "false", page_token: pageToken, filters: JSON.stringify({phase: {eq: "PHASE_TYPE_COMPLETE"}, trashed: {eq: false}})});
    ...
    private fileToObj(f: PikPakFile): fso.FileInfo {
    ...
    async downFile(file?: fso.FileFind): Promise<fso.FileLink[]> {
        try {
            ...
            const resp = await this.clouds.request(`${con.API_URL}${con.API_ENDPOINTS.FILES}/${file.uuid}?_magic=2021&usage=FETCH&thumbnail_size=SIZE_LARGE`);
            let url = resp.web_content_link || "";
            if (!this.config.disable_media_link && resp.medias?.length > 0 && resp.medias[0].link?.url) url = resp.medias[0].link.url;
    ...
    async copyFile(file?: fso.FileFind, dest?: fso.FileFind): Promise<fso.FileTask> {
        try {
            ...
            await this.clouds.request(`${con.API_URL}${con.API_ENDPOINTS.BATCH_COPY}`, "POST", {ids: [file.uuid], to: {parent_id: dest.uuid}});
    ...
    async moveFile(file?: fso.FileFind, dest?: fso.FileFind): Promise<fso.FileTask> {
        try {
            if (!file?.uuid || !dest?.uuid) throw new Error("文件ID不能为空");
            await this.clouds.request(`${con.API_URL}${con.API_ENDPOINTS.BATCH_MOVE}`, "POST", {ids: [file.uuid], to: {parent_id: dest.uuid}});
    ...
    async killFile(file?: fso.FileFind): Promise<fso.FileTask> {
        try {
            ...
            await this.clouds.request(`${con.API_URL}${con.API_ENDPOINTS.BATCH_TRASH}`, "POST", {ids: [file.uuid]});
    ...
    async makeFile(file?: fso.FileFind, name?: string | null, type?: fso.FileType, data?: any | null): Promise<DriveResult> {
        try {
            if (type === fso.FileType.F_DIR) {
                await this.clouds.request(`${con.API_URL}${con.API_ENDPOINTS.FILES}`, "POST", {kind: "drive#folder", parent_id: file?.uuid || "", name});
